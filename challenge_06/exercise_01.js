...
let astronomy = weatherData.query.results.channel.astronomy;
console.log(`Sunrise is at: ${astronomy.sunrise}`);
console.log(`Sunset is at: ${astronomy.sunset}`);

let forecast = weatherData.query.results.channel.item.forecast;

for(let day = 0; day<7; day++){
    info = forecast[day.toString()];
    console.log(`${info.day}: ${info.text}. MAX: ${fahrenheitToCelcius(info.high)}°C MIN: ${fahrenheitToCelcius(info.low)}°C`);
}
...
function padHour(hour){
    let detail = hour.split(":");
    if (detail[1].length < 2)
        detail[1]+="0";
    return detail.join(":");
}
